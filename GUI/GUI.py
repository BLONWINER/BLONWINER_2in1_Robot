...
def call_LeftBackward(event):            #When this function is called,client commands the car to turn left
    global c_l_stu
    if c_l_stu == 0:
        tcpClicSock.send(('leftBackward').encode())
        c_l_stu=1


def call_RightBackward(event):           #When this function is called,client commands the car to turn right
    global c_r_stu
    if c_r_stu == 0:
        tcpClicSock.send(('rightBackward').encode())
        c_r_stu=1


def call_OutSide(event):
    tcpClicSock.send(('Out').encode())


def call_InSide(event):
    tcpClicSock.send(('In').encode())
    
    
def call_LeftForward(event):
    tcpClicSock.send(('leftForward').encode())
    
def call_RightForward(event):
    tcpClicSock.send(('rightForward').encode())
...
def instruction():
    ...
    while 1:
        ...
        instruction_2 = 'W:Forward  S:Backward  A:Left Backward  D:Right Backward '
        instructions.append(instruction_2)
        instruction_3 = 'Q:Left Forward  E:Right Forward  I:Look up K:Look down  J:Grab  '
        instructions.append(instruction_3)
        instruction_4 = 'L:Loose   Q:Hand reaches out   E:Hand takes back   U&O:Hand rotation'
...
def loop():                      #GUI
    ...
    while True:
        color_bg='#4A5063'        #Set background color
        color_text='#E1F5FE'      #Set text color
        color_btn='#787D91'       #Set button color
        color_line='#787D91'      #Set line color
        color_can='#787D91'       #Set canvas color
        color_oval='#787D91'      #Set oval color
        target_color='#FF6D00'

        root = tk.Tk()            #Define a window named root
        root.title('BLONWINER 2in1 Robot')      #Main window title
        root.geometry('565x580')  #Main window size, middle of the English letter x.565x510
        root.config(bg=color_bg)  #Set the background color of root window

        try:
            logo =tk.PhotoImage(file = 'BLONWINER.png')         #Define the picture of logo,but only supports '.png' and '.gif'
            l_logo=tk.Label(root,image = logo,bg=color_bg) #Set a label to show the logo picture
            
            l_logo.place(x=20,y=5)                        #Place the Label in a right position
        ...
        CPU_TEP_lab=tk.Label(root,width=18,text='CPU Temp:',fg=color_text,bg='#787D91')
        CPU_TEP_lab.place(x=400,y=15)                         #Define a Label and put it in position

        CPU_USE_lab=tk.Label(root,width=18,text='CPU Usage:',fg=color_text,bg='#787D91')
        CPU_USE_lab.place(x=400,y=45)                         #Define a Label and put it in position

        RAM_lab=tk.Label(root,width=18,text='RAM Usage:',fg=color_text,bg='#787D91')
        ...
        label_ins.place(x=30,y=530)                         #Define a Label and put it in position

        E1 = tk.Entry(root,show=None,width=16,bg="#4A5063",fg='#eceff1')
        E1.place(x=190,y=65)                             #Define a Entry and put it in position

        l_ip_3=tk.Label(root,width=29,text='Please enter the Raspberry Pi ',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=100,y=25)                         #Define a Label and put it in position   ,font=('Arial', 16)
        l_ip_3=tk.Label(root,width=10,text='IP address:',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=112,y=65)                         #Define a Label and put it in position   ,font=('Arial', 16)
        ...
        l_ip_3=tk.Label(root,width=22,text='Robotic motor control:',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=15,y=180)                         #Define a Label and put it in position
        l_ip_3=tk.Label(root,width=22,text='Robotic arm control:',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=280,y=180)                         #Define a Label and put it in position
        l_ip_3=tk.Label(root,width=16,text='Tips for action:',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=10,y=500)                         #Define a Label and put it in position
        l_ip_3=tk.Label(root,width=29,text='WS2812 RGB Lamp Color Setting:',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=19,y=308)                         #Define a Label and put it in position
        ...
        Btn2 = tk.Button(root, width=8, text='LeftB',fg=color_text,bg=color_btn,relief='ridge')
        Btn3 = tk.Button(root, width=8, text='RightB',fg=color_text,bg=color_btn,relief='ridge')

        Btn_LeftF = tk.Button(root, width=8, text='LeftF',fg=color_text,bg=color_btn,relief='ridge')
        Btn_LeftF.place(x=30,y=210)
        Btn_LeftF.bind('<ButtonPress-1>', call_LeftForward)
        Btn_LeftF.bind('<ButtonRelease-1>', call_Turn_stop)

        Btn_RightF = tk.Button(root, width=8, text='RightF',fg=color_text,bg=color_btn,relief='ridge')
        Btn_RightF.place(x=170,y=210)
        Btn_RightF.bind('<ButtonPress-1>', call_RightForward)
        Btn_RightF.bind('<ButtonRelease-1>', call_Turn_stop)
        
        Btn_OutSide = tk.Button(root, width=8, text='Out',fg=color_text,bg=color_btn,relief='ridge')
        Btn_OutSide.place(x=300,y=280)
        Btn_OutSide.bind('<ButtonPress-1>', call_OutSide)
        Btn_OutSide.bind('<ButtonRelease-1>', call_Turn_stop)
    
        Btn_InSide = tk.Button(root, width=8, text='In',fg=color_text,bg=color_btn,relief='ridge')
        Btn_InSide.place(x=440,y=280)
        Btn_InSide.bind('<ButtonPress-1>', call_InSide)
        Btn_InSide.bind('<ButtonRelease-1>', call_Turn_stop)

        Btn0.place(x=100,y=210)
        Btn1.place(x=100,y=245)
        Btn2.place(x=30,y=245)
        Btn3.place(x=170,y=245)

        Btn0.bind('<ButtonPress-1>', call_forward)
        Btn1.bind('<ButtonPress-1>', call_back)
        Btn2.bind('<ButtonPress-1>', call_LeftBackward)
        Btn3.bind('<ButtonPress-1>', call_RightBackward)
        ...
        root.bind('<KeyPress-a>', call_LeftBackward)
        root.bind('<KeyPress-d>', call_RightBackward)
        root.bind('<KeyPress-s>', call_back)

        root.bind('<KeyPress-q>', call_LeftForward)
        root.bind('<KeyPress-e>', call_RightForward)
        root.bind('<KeyRelease-q>', call_Turn_stop)
        root.bind('<KeyRelease-e>', call_Turn_stop)
		
        root.bind('<KeyPress-n>', call_OutSide)
        root.bind('<KeyPress-m>', call_InSide)
        root.bind('<KeyRelease-n>', call_Turn_stop)
        root.bind('<KeyRelease-m>', call_Turn_stop)
        ...
        Btn_up.place(x=370,y=210)
        Btn_down.place(x=370,y=245)
        Btn_left.place(x=300,y=245)
        Btn_right.place(x=440,y=245)
        Btn_home.place(x=370,y=280)
        ...
        Btn_Cleft.place(x=300, y=210)
        Btn_Cright.place(x=440, y=210)
        ...
        Btn14.place(x=310,y=61)                          #Define a Button and put it in position  height=2,   
        ...
        from_=0,to=255,orient=tk.HORIZONTAL,length=450,
        showvalue=1,tickinterval=None,resolution=1,variable=var_R,troughcolor='#787D91',command=set_R,fg=color_text,bg=color_bg,highlightthickness=0)
        Scale_R.place(x=80,y=330)                            #Define a Scale and put it in position
        ...
        from_=0,to=255,orient=tk.HORIZONTAL,length=450,
        showvalue=1,tickinterval=None,resolution=1,variable=var_G,troughcolor='#787D91',command=set_G,fg=color_text,bg=color_bg,highlightthickness=0)
        Scale_G.place(x=80,y=360)                            #Define a Scale and put it in position
        ...
        from_=0,to=255,orient=tk.HORIZONTAL,length=450,
        showvalue=1,tickinterval=None,resolution=1,variable=var_B,troughcolor='#787D91',command=set_B,fg=color_text,bg=color_bg,highlightthickness=0)
        Scale_B.place(x=80,y=390)                            #Define a Scale and put it in position

        canvas_cover=tk.Canvas(root,bg=color_bg,height=30,width=510,highlightthickness=0)
        canvas_cover.place(x=80,y=420)
        l_ip_3=tk.Label(root,width=8,text='B Value:',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=20,y=390)                         #Define a Label and put it in position
        l_ip_3=tk.Label(root,width=8,text='G Value:',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=20,y=360)                         #Define a Label and put it in position     
        l_ip_3=tk.Label(root,width=8,text='R Value:',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=20,y=330)                         #Define a Label and put it in position 
        
        l_ip_3=tk.Label(root,width=16,text='Functional mode:',fg=color_text,bg='#4A5063')
        l_ip_3.place(x=19,y=430)                         #Define a Label and put it in position
        

        Btn_Steady = tk.Button(root, width=10, text='Ultrasonic',fg=color_text,bg=color_btn,relief='ridge')
        Btn_Steady.place(x=30,y=460)
        ...
        Btn_FindColor.place(x=170,y=460)
        ...
        Btn_WatchDog.place(x=310,y=460)
